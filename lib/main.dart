...
class _MyHomePageState extends State<MyHomePage> {
  ...
  Future<List<RecorridoA>> getRecorridos() async {
    final response = await http.get(Uri.parse("http://10.0.2.2:8000/api/recorrido"));
  ...
  Future<List<EventoA>> getEventos() async {
    final response = await http.get(Uri.parse("http://10.0.2.2:8000/api/evento"));
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      key: cuponesListKey,
      appBar: AppBar(title: Text('AnimalFav'), actions: <Widget>[
        TextButton(
          ...
          onPressed: () {
            Navigator.push(
              context,
              MaterialPageRoute(builder: (context) => const MyHomePage(title: 'AnimalFav',)),
            );
          },
        ...
        TextButton(
          ...
          onPressed: () {
            Navigator.push(
              context,
              MaterialPageRoute(builder: (context) => const CuponesPage(title: 'AnimalFav',)),
            );
          },
                ...
                child: Column(
                    ...
                    children: [
                      ...
                      Row(
                        mainAxisAlignment: MainAxisAlignment.start,
                        children: [
                          const Text("Eventos",style: TextStyle(fontWeight: FontWeight.bold, fontSize: 20),)
                        ],),]
                ),),
                ...
                child: Column(
                    ...
                    children: [
                      Row(
                        mainAxisAlignment: MainAxisAlignment.start,
                        children: [
                          const Text("Recorridos",style: TextStyle(fontWeight: FontWeight.bold, fontSize: 20),)
                        ],),]
                ),),
                        ...
                        child: ListTile(
                          title: Text(
                            "Patrocinador: " + data.nombre + "\n" +
                                ...
                                "Dia: " + data.dia + "\n" +
                                "Duración: " +data.duracion + "\n" +
                                "Descripción del recorrido: " + data.descRecorrido,
                            style: const TextStyle(fontSize: 20),
                          ),
                        ),
                      );
...
Future<http.Response> crearRecorrido(nombre, correo, dia, duracion, ubicacion, descRecorrido) {
  return http.post(
    Uri.parse("http://10.0.2.2:8000/api/evento/"),
    headers: <String, String>{
      'Content-Type': 'application/json; charset=UTF-8',
...
class Evento extends StatelessWidget{
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        appBar: AppBar(title: Text('Creación de Eventos'), actions: <Widget>[
          TextButton(
            style: TextButton.styleFrom(
                primary: Theme.of(context).colorScheme.onPrimary),
            onPressed: () {
            ...
            child: Column(
              ...
              children: [
                ...
                TextField(
                    ...
                    decoration: InputDecoration(
                        border: OutlineInputBorder(), hintText: 'Nombre Evento')),
                TextField(
                    controller: diaController,
                    decoration: InputDecoration(
                        border: OutlineInputBorder(), hintText: 'Dia y Hora (aaaa-mm-ddTHH:MM:SS)')),
                TextField(
                    controller: duracionController,
                    decoration: InputDecoration(
                        border: OutlineInputBorder(), hintText: '60 minutos')),
                TextField(
                    controller: ubicacionController,
                    decoration: InputDecoration(
                        border: OutlineInputBorder(), hintText: 'Ubicación del Evento')),
                TextButton(
                  onPressed: () {
                    crearEvento(nombreController, correoController, diaController, duracionController, ubicacionController);
            ...
            child: Column(
              ...
              children: [
                ...
                const TextField(
                    obscureText: true,
                    decoration: InputDecoration(
                        border: OutlineInputBorder(), hintText: 'Nombre del Recorrido')),
                const TextField(
                    obscureText: true,
                    decoration: InputDecoration(
                        border: OutlineInputBorder(), hintText: 'Dia y Hora (dd-mm-aaaa HH:MM:SS)')),
                const TextField(
                    obscureText: true,
                    decoration: InputDecoration(
                        border: OutlineInputBorder(), hintText: 'Ubicación del Recorrido')),
                const TextField(
                    obscureText: true,
                    decoration: InputDecoration(
                        border: OutlineInputBorder(), hintText: 'Descripción del Recorrido')),
                TextButton(
                  onPressed: () {
                    crearRecorrido(nombreController, correoController, diaController, duracionController, ubicacionController, descRecorridoControler);
                    ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(content: Text('Datos Enviados')));
                  },
                  child: const Text('Enviar'),
